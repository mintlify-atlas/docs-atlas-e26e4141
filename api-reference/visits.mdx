---
title: "Visits API: Raw Visit Log for Links and Domains"
sidebarTitle: "Visits"
...
<ParamField query="to" type="string (ISO 8601 datetime)">
  Return only visits that occurred at or before this instant. When a bare date is supplied (e.g. `2024-05-31`), it is interpreted as the end of that calendar day (exclusive upper bound on the next day). Inclusive when a full datetime is provided.
</ParamField>
...
<ParamField query="os" type="string">
  Filter to a specific OS. Case-insensitive substring match against the stored value (e.g. `windows`, `macos`, `linux`). Open-ended — not a closed enum.
</ParamField>

<ParamField query="browser" type="string">
  Filter to a specific browser. Case-insensitive substring match (e.g. `chrome`, `safari`, `firefox`). Open-ended — not a closed enum.
</ParamField>
