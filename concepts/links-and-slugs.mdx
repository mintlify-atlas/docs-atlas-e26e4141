---
title: "Links and Slugs: How Short URLs Work in linq"
...
<Tabs>
  ...
  <Tab title="tree">
    A `tree` link serves a **hosted HTML page** instead of a redirect. The page displays a title, description, optional image, and an ordered list of items — each a label pointing to another link on the same domain. There is no `destination` field on a tree link. See [Link Trees](/concepts/link-trees) for details.
  </Tab>
</Tabs>
...
<ResponseField name="destination" type="string (URI)">
  The long URL visitors are sent to. Present on `redirect` links; absent on `tree` links.
</ResponseField>
